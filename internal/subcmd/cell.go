...
func Cell(home string, args []string) error {
	if len(args) < 1 {
		return fmt.Errorf("usage: mforge cell <add|bootstrap> ...")
	}
	op := args[0]
	rest := args[1:]
	switch op {
	case "add":
		...
		if _, err := store.CreateCell(db, rigRow.ID, cellName, scope, worktree); err != nil {
			return err
		}
	...
	case "bootstrap":
		if len(rest) < 2 {
			return fmt.Errorf("usage: mforge cell bootstrap <rig> <cell> [--architect]")
		}
		rigName, cellName := rest[0], rest[1]
		withArchitect := false
		for i := 2; i < len(rest); i++ {
		...
		for _, role := range roles {
			...
			if err := util.AtomicWriteFile(store.CellRoleMetaPath(home, rigName, cellName, role), b, 0o644); err != nil {
				return err
			}
