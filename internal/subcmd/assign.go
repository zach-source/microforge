...
func Assign(home string, args []string) error {
	if len(args) < 1 { return fmt.Errorf("usage: mf assign <rig> --task <id> --cell <cell> --role <role>") }
	rigName := args[0]
	...
	for i := 1; i < len(args); i++ {
		switch args[i] {
		case "--task":
			if i+1 < len(args) { taskID = args[i+1]; i++ }
		case "--cell":
			if i+1 < len(args) { cellName = args[i+1]; i++ }
		case "--role":
			if i+1 < len(args) { role = args[i+1]; i++ }
		case "--promise":
			if i+1 < len(args) { promise = args[i+1]; i++ }
		}
	}
	...
	if _, err := store.CreateAssignment(db, rigRow.ID, taskID, agentRow.ID, inboxRel, outboxRel, promise, nil); err != nil { return err }
